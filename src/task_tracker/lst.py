...
from .helpers.helpers import print_entries, print_description, define_idx, pkg_path, data_path, check_init
...
def main():
    ...
    if d["ref_proj"] == "ALL":
        ...
        df = pd.read_csv(f"{data_path}/projects/{d['ref_proj']}/{d['file']}.csv")
        if d["pos"] is None:
            print_entries(df)
        else:
